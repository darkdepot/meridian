...
describe("PrewarmPlanStore", () => {
  test("reports disabled before looking up a session", () => {
    ...
    expect(plans.prepare("unknown")).toEqual({ status: "disabled" })
  })
  ...
  test("prepares a known session idempotently and rejects an unknown session", () => {
    ...
    const plans = new PrewarmPlanStore(pool, 2)
    plans.register("known", { key: "warm-key", options: {} })

    expect(plans.prepare("known")).toEqual({ status: "warming" })
    expect(plans.prepare("known")).toEqual({ status: "already_warm" })
    expect(plans.prepare("unknown")).toEqual({ status: "unknown_session" })
    pool.closeAll()
  })
  ...
  test("bounds remembered plans", () => {
    ...
    plans.register("old", { key: "old-key", options: {} })
    plans.register("new", { key: "new-key", options: {} })

    expect(plans.prepare("old")).toEqual({ status: "unknown_session" })
    expect(plans.prepare("new")).toEqual({ status: "warming" })
    pool.closeAll()
  })
})
